...
#if NAPT_USE_HOOK
static alg_hook_callback alg_tcp_input_hook  = NULL;
static alg_hook_callback alg_udp_input_hook  = NULL;
static alg_hook_callback alg_tcp_output_hook = NULL;
static alg_hook_callback alg_udp_output_hook = NULL;
#endif

#if NAPT_USE_HOOK
/*****************************************************************************
 Prototype    : alg_set_input_hook
 Description  : packet from internet to station
 Note         : excluding packet of to apsta gateway
*****************************************************************************/
void alg_set_input_hook(alg_hook_type_e type, alg_hook_callback input)
{
    if (ALG_HOOK_TYPE_TCP == type)
        alg_tcp_input_hook = input;
    else if (ALG_HOOK_TYPE_UDP == type)
        alg_udp_input_hook = input;

    return;
}

/*****************************************************************************
 Prototype    : alg_set_output_hook
 Description  : packet from station to internet
 Note         : excluding LAN
*****************************************************************************/
void alg_set_output_hook(alg_hook_type_e type, alg_hook_callback output)
{
    if (ALG_HOOK_TYPE_TCP == type)
        alg_tcp_output_hook = output;
    else if (ALG_HOOK_TYPE_UDP == type)
        alg_udp_output_hook = output;
...
#if NAPT_USE_HOOK
        if (alg_tcp_output_hook)
        {
            alg_hook_action_e ret = alg_tcp_output_hook(ehdr, eth_len);
            if (ret)
                return ret;
        }
#endif
...
#if NAPT_USE_HOOK
            if (alg_tcp_input_hook)
            {
                alg_hook_action_e ret = alg_tcp_input_hook(ehdr, eth_len);
                if (ret)
                    return ret;
            }
#endif
...
#if NAPT_USE_HOOK
            if (alg_udp_output_hook)
            {
                alg_hook_action_e ret = alg_udp_output_hook(ehdr, eth_len);
                if (ret)
                    return ret;
            }
#endif
...
#if NAPT_USE_HOOK
            if (alg_udp_input_hook)
            {
                alg_hook_action_e ret = alg_udp_input_hook(ehdr, eth_len);
                if (ret)
                    return ret;
            }
#endif
